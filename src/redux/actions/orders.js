...
export const createOrder = (order) => async (dispatch)=>{
    ...
    try{
        const res = await axios.post(`${URL}/user/payment/orders`, order);
        console.log(res);
...
export const placeOrder = (order) => async (dispatch, getState)=>{
    dispatch({type: CREATE_ORDER_REQ, payload: {}});
    try{
        const res = await axios.post(`${URL}/user/payment/success`, order);
        if(res.data.status === 200){
            dispatch({type: ORDER_SUCCESS, payload: res.data.success.data});
...
export const getOders = (id) => async (dispatch)=>{
    ...
    try{
        const res = await axios.get(`${URL}/user/get-order/${id}`)
        console.log(res);
        if(res.data.status === 200){
            dispatch({type: ORDER_SUCCESS, payload:res.data.success.data})
        }
